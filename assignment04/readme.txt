...
2) On the login page (at /), enter the following "' OR 1=1 --" in the password field and click login
...
The <script> tag is injected into the DOM without sanitization, causing an alert to be executed.

3. Exploit Reflected XSS

The flash messages on the site are vulnerable to reflected XSS. One such entry point is the search functionality.

1) Navigate to /dashboard and use the search form.
...
Once you retrieve the superadmin password from the SQL injection in Task 4, you can log in as the administrator.

1) Navigate to http://127.0.0.1:8080/admin.
2) Enter the superadmin password that you retrieved from the previous step into the form and submit it.


6. Exploit Open Redirect

The goto route is vulnerable to an open redirect. You can redirect users to arbitrary websites by modifying the to parameter.
...
1)Navigate to /admin.
2)In the filename field, use the following payload to attempt reading a sensitive file:

Payload (in URL):

http://127.0.0.1:8080/admin?show=../../../../etc/passwd

This URL attempts to traverse the directory and access /etc/passwd. If the server permits file traversal and does not sanitize file paths properly, it will return the file contents.
